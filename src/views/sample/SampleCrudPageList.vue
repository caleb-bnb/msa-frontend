  ...
  <b-card>
    <b-container class="bv-example-row">
      <b-row class="text-center">
        ...
        <b-col cols="4">
          <BComFormInput v-model="searchParams.storeName" placeholder="중개업소명을 입력해 주세요." />
        </b-col>
      </b-row>
      <b-row class="text-center">
...
export default {
  ...
  methods: {
    getSearchParams(method) {
      ...
      _searchParams = Object.assign(_searchParams, this.page)
    ...
    onSearch() {
      api.getList('/v1/sample/pageList', this.getSearchParams('get'))
      .then((res) => {
        console.log(res)
        this.grid.items = res.data.content
        this.page.totalElements = res.data.totalElements
        this.page.totalPages = res.data.totalPages
      })
    },
    onSearchPage(button, pageNo) {
