<!--
  참조url : https://bootstrap-vue.org/docs/components/table
-->
<template>
  <b-container class="bv-example-row">
    <b-row>
      <b-col class="b-head top-border">아이디</b-col>
      <b-col cols="9" class="b-body top-border">
        <BComFormInput v-model="registParams.userId" refs="userId" />
      </b-col>
    </b-row>
    <b-row>
      <b-col class="b-head">회원명</b-col>
      <b-col cols="9" class="b-body">
        <BComFormInput v-model="registParams.userName" ref="userName" />
      </b-col>
    </b-row>
    <b-row>
      <b-col class="b-head">중개업소명</b-col>
      <b-col cols="9" class="b-body">
        <BComFormInput v-model="registParams.storeName" ref="storeName" />
      </b-col>
    </b-row>
    <b-row>
      <b-col class="b-head">회원번호</b-col>
      <b-col cols="9" class="b-body">
        <BComFormInput v-model="registParams.userNo" ref="userNo" />
      </b-col>
    </b-row>
    <b-row>
      <b-col class="b-head">설명</b-col>
      <b-col cols="9" class="b-body">
        <BComFormTextarea
          v-model="registParams.description"
          ref="description"
          placeholder="설명을 입력해 주세요."
        />
      </b-col>
    </b-row>
    <b-row>
      <b-col class="b-head">사용여부</b-col>
      <b-col cols="9" class="b-body">
        <BComFormRadio
          v-model="registParams.useYn"
          ref="useYn"
          :value="registParams.useYn"
          :options="useYnOptions"
          @return="returnUseYn"
        />
      </b-col>
    </b-row>
    <b-row>
      <b-col class="b-head">그룹코드</b-col>
      <b-col cols="9" class="b-body">
        <div style="float:left">
          <BComFormInput
            v-model="registParams.groupCode"
            ref="groupCode"
            placeholder="Readonly"
            readonly
          />
        </div>
        <div style="float:left">
          <BComFormInput
            v-model="registParams.groupName"
            ref="groupName"
            placeholder="Readonly"
            readonly
          />
        </div>
        <div style="float:left">
          <b-button
            @click="isVisibleGroupCodePopup=true"
          >검색
          </b-button>
          <BSampleGroupCodePopup
            v-model="isVisibleGroupCodePopup"
            title="그룹코드 조회"
            @modal-cancel="isVisibleGroupCodePopup=false"
            @return="selectedGroupCode"
          />
        </div>
      </b-col>
    </b-row>
    <div style="margin:10px; padding:10px; text-align:center;">
      <b-button @click="onSave">저장</b-button>
      <b-button variant="outline-primary" @click="onBtnList">목록</b-button>
      <b-button variant="outline-primary" @click="onBtnPageList">목록(페이징)</b-button>
    </div>
    <div>
      * 결과
      <div style="padding:10px; border:1px solid #CCCCCC; color:#AAA;">
        {{ this.registParams }}
      </div>
    </div>
  </b-container>
</template>

<script>
import _ from 'lodash'
import BComFormInput from '@/components/bootstrap/BComFormInput'
import BComFormTextarea from '@/components/bootstrap/BComFormTextarea'
import BComFormRadio from '@/components/bootstrap/BComFormRadio'
import BSampleGroupCodePopup from '@/views/sample/popup/bootstrap/BSampleGroupCodePopup'
import { api } from '@/api/sample'

export default {
  name: 'SampleCrudCreate',
  components: {
    BComFormInput,
    BComFormTextarea,
    BComFormRadio,
    BSampleGroupCodePopup,
  },
  data() {
    return {
      params: this.$route.params,  // [수정]일 경우 목록에서 선택한 row의 값이 저장되는 변수
      registParams: {
        userId: '',
        userName: '',
        storeName: '',
        userNo: '',
        description: '',
        useYn: 'N',
        groupCode: '',
        groupName: '',
      },
      useYnOptions: [
        { value: 'Y', text: '사용' },
        { value: 'N', text: '미사용' },
      ],
      isVisibleGroupCodePopup: false,
    }
  },
  mounted() {
    console.log('SampleCrudCreate')
    console.log('registParams.useYn='+ this.$route.query.useYn)
    this.setParams()
  },
  methods: {
    // this.$route.params로 넘어온 값을 세팅한다.
    setParams() {
      this.registParams.userId = this.$route.query.userId
      this.registParams.userName = this.$route.query.userName
      this.registParams.storeName = this.$route.query.storeName
      this.registParams.userNo = this.$route.query.userNo
      this.registParams.description = this.$route.query.description
      this.registParams.useYn = this.$route.query.useYn
      this.registParams.groupCode = this.$route.query.groupCode
      this.registParams.groupName = this.$route.query.groupName
    },
    returnUseYn(res) {
      this.registParams.useYn = res
    },
    closePopup() {
      this.refs['BSampleGroupCodePopup'].hide()
    },
    selectedGroupCode(row) {
      this.registParams.groupCode = row.item.groupCodeId
      this.registParams.groupName = row.item.groupCodeName
    },
    onSaveValidate() {
      if(_.isEmpty(this.registParams.userId)) {
        alert('아이디를 입력해 주세요')
        return false
      }
      if(_.isEmpty(this.registParams.userNo)) {
        alert('회원번호를 입력해 주세요')
        return false
      }

      return true
    },
    onSave() {
      if(this.onSaveValidate() === false) {
        return
      }

      api.saveUser('/v1/sample/save', this.registParams).then((res) => {
        console.log(res)
        alert('저장되었습니다.')
        this.$router.push('/sample/crudList')
      }).catch(error => {
        console.log(error.response)
      })
    },
    onBtnList() {
      this.$router.push('/sample/crudList')
    },
    onBtnPageList() {
      this.$router.push('/sample/crudPageList')
    },
  }
}
</script>

<style scoped>
  .b-head {
    background-color: #EFEFEF;
    border-bottom: 1px solid #CCCCCC;
    border-left: 1px solid #CCCCCC;
    padding: 5px 10px;
  }
  .b-body {
    border-bottom: 1px solid #CCCCCC;
    border-right: 1px solid #CCCCCC;
    padding: 2px 5px;
  }
  .top-border {
    border-top: 1px solid #CCCCCC;
  }
</style>